...
int main()
{
    ...
    // note: we suggest use of 2 threads - normally one is fin (we are simply demonstrating thread
    // safety).
    asio::io_service service(4);

    // create a work object to process our events.
    asio::io_service::work work(service);

    // handler for libev
    AMQP::LibAsioHandler handler(service);
    ...
    // run the handler
    // a t the moment, one will need SIGINT to stop.  In time, should add signal handling through
    // boost API.
    return service.run();
}
