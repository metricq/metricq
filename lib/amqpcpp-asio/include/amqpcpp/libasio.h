 ...
 *  LibAsio.h
 *
 *  Implementation for the AMQP::TcpHandler that is optimized for asio. You can
 *  use this class instead of a AMQP::TcpHandler class, just pass the asio service
...
/**
 *  Class definition
 *  @note Because of a limitation on Windows, this will only work on POSIX based systems - see
 * https://github.com/chriskohlhoff/asio/issues/70
 */
class LibAsioHandler : public virtual TcpHandler
{
private:
    /**
     *  Helper class that wraps a io_service socket monitor.
     */
    class Watcher : public virtual std::enable_shared_from_this<Watcher>
    {
    private:
        /**
         *  The asio io_service which is responsible for detecting events.
         *  @var class asio::io_service&
         */
        asio::io_service& _ioservice;

        typedef std::weak_ptr<asio::io_service::strand> strand_weak_ptr;

        /**
         *  The asio io_service::strand managed pointer.
         ...
         *  @note
         * https://stackoverflow.com/questions/38906711/destroying-boost-asio-socket-without-closing-native-handler
         */
        ...
        using handler_cb = std::function<void(std::error_code, std::size_t)>;
        using io_handler = std::function<void(const std::error_code&, const std::size_t)>;

        /**
         * Builds a io handler callback that executes the io callback in a strand.
         * @param  io_handler  The handler callback to dispatch
         * @return handler_cb  A function wrapping the execution of the handler function in a
         * io_service::strand.
         */
        handler_cb get_dispatch_wrapper(io_handler fn)
        {
            ...
            return [fn, wpstrand](const std::error_code& ec, const std::size_t bytes_transferred) {
                const strand_shared_ptr strand = wpstrand.lock();
                if (!strand)
                {
                    fn(std::make_error_code(std::errc::operation_canceled), std::size_t{ 0 });
                    return;
                }
                strand->dispatch(std::bind(fn, ec, bytes_transferred));
            };
        }
        ...
        /**
         *  Handler method that is called by asio's io_service when the socket pumps a read event.
         ...
         */
        void read_handler(const std::error_code& ec, const std::size_t bytes_transferred,
                          const std::weak_ptr<Watcher> awpWatcher, TcpConnection* const connection,
                          const int fd)
        {
        ...
        /**
         *  Handler method that is called by asio's io_service when the socket pumps a write event.
         *  @param  ec          The status of the callback.
         *  @param  bytes_transferred The number of bytes transferred.
         *  @param  awpWatcher  A weak pointer to this object.
         *  @param  connection  The connection being watched.
         ...
         */
        void write_handler(const std::error_code ec, const std::size_t bytes_transferred,
                           const std::weak_ptr<Watcher> awpWatcher, TcpConnection* const connection,
                           const int fd)
        {
            // Resolve any potential problems with dangling pointers
    ...
    public:
         ...
         *  a asio socket for monitoring.
         *  @param  io_service      The asio io_service
         *  @param  wpstrand        A weak pointer to a io_service::strand instance.
         *  @param  fd              The filedescriptor being watched
         */
        Watcher(asio::io_service& io_service, const strand_weak_ptr wpstrand, const int fd)
        : _ioservice(io_service), _wpstrand(wpstrand), _socket(_ioservice)
        {
            _socket.assign(fd);
    ...
    private:
        ...
        typedef asio::basic_waitable_timer<std::chrono::system_clock> asio_waitable_timer;

        /**
         *  The asynchronous deadline timer.
         *  @var class asio::deadline_timer
         */
        asio_waitable_timer _timer;

        using handler_fn = std::function<void(std::error_code)>;
        /**
        ...
        handler_fn get_handler(TcpConnection* const connection, const uint16_t timeout)
        {
            ...
            return [fn, wpstrand](const std::error_code& ec) {
                const strand_shared_ptr strand = wpstrand.lock();
                if (!strand)
                {
                ...
                strand->dispatch(std::bind(fn, ec));
            };
        }
         ...
         *  @param  timeout
         */
        void timeout(const std::error_code& ec, std::weak_ptr<Timer> awpThis,
                     TcpConnection* const connection, const uint16_t timeout)
        {
            // Resolve any potential problems with dangling pointers
            // (remember we are using async).
    ...
    public:
         ...
         *  @param  wpstrand   A weak pointer to a io_service::strand instance.
         */
        Timer(asio::io_service& io_service, const strand_weak_ptr wpstrand)
        : _ioservice(io_service), _wpstrand(wpstrand), _timer(_ioservice)
        {
        }
    ...
    typedef std::shared_ptr<asio::io_service::strand> strand_shared_ptr;
    ...
    /**
     *  Method that is called by AMQP-CPP to register a filedescriptor for readability or
     * writability
     *  @param  connection  The TCP connection object that is reporting
     *  @param  fd          The filedescriptor to be monitored
     *  @param  flags       Should the object be monitored for readability or writability?
     */
    void monitor(TcpConnection* const connection, const int fd, const int flags) override
    {
...
protected:
    /**
     *  Method that is called when the heartbeat frequency is negotiated between the server and the
     * client.
...
public:
     ...
     *  @param  that    The object to not move or copy
     */
    LibAsioHandler() = delete;

    /**
     *  Constructor
     *  @param  io_service    The io_service to wrap
     */
    explicit LibAsioHandler(asio::io_service& io_service)
    : _ioservice(io_service), _strand(std::make_shared<asio::io_service::strand>(_ioservice)),
     ...
     *  @param  that    The object to not move or copy
     */
    LibAsioHandler(LibAsioHandler&& that) = delete;
    LibAsioHandler(const LibAsioHandler& that) = delete;

    /**
     *  Returns a reference to the io_service object that is being used.
     *  @return The io_service object.
     */
     ...
     *  Destructor
     */
    ~LibAsioHandler() override = default;
};
